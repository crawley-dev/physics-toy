...
//     return textureSample(texture_data, texture_sampler, uv); // This returns a vec4<f32>
// }
...
fn fs_main(@builtin(position) fragCoord: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    let colour = vec3<f32>(r, g, b);
    // let colour = textureSample(texture_data, texture_sampler, uv_scaled);
