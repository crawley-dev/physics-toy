...
fn main() {
    ...
    const INIT_HEIGHT: u32 = 400;
    const INIT_TITLE: &str = "Hello, World!";
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let window = Arc::new(
            event_loop
                .create_window(self.window_attributes.take().unwrap())
                .unwrap(),
        );
        ...
        // This ensures the GPU can load the correct surface.
        let compatible_surface = instance.create_surface(window.clone()).unwrap();
        info!("Compatible surface created: {compatible_surface:?}");
        ...
        let mut renderer = instance.create
    }
