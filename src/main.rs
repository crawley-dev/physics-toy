...
impl App {
    fn new(title: &str, width: u32, height: u32) -> Self {
        Self {
            ...
            window_attributes: Some(Window::default_attributes().with_title(title).with_inner_size(LogicalSize::new(width, height))),
            timer: Instant::now(),
        }
    }
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        let window_attributes = self.window_attributes.take().unwrap_or_else(Window::default_attributes);
    ...
    fn window_event(&mut self, event_loop: &ActiveEventLoop, id: WindowId, event: WindowEvent) {
        match event {
            ...
            WindowEvent::RedrawRequested => {
                ...
                let remaining_frame_time = (FRAME_TIME_MS - self.timer.elapsed().as_millis_f64()).clamp(0.0, FRAME_TIME_MS);
                std::thread::sleep(std::time::Duration::from_millis(remaining_frame_time as u64));
            }
