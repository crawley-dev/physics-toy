...
pub const OUTPUT_EVERY_N_FRAMES: u64 = 60;
...
macro_rules! create_vec2 {
    ($name:ident, $param1:ident, $param2: ident) => {
        ...
        pub struct $name<T: num::Num> {
            pub $param1: T,
            pub $param2: T,
        }
        impl<T: num::Num> $name<T> {
            pub fn new($param1: T, $param2: T) -> Self {
                Self { $param1, $param2 }
            }
        }
        impl<T: num::Num> From<PhysicalSize<T>> for $name<T> {
        ...
        impl<T: num::Num> From<PhysicalPosition<T>> for $name<T> {
